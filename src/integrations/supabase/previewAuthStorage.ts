...
// postMessage so the project's preview surfaces share one login; else sessionStorage.
export function brokeredPreviewStorage() {
  if (typeof window === 'undefined') return undefined;
  const browserStorage = window.sessionStorage;

  // Remove sessions created by the previous localStorage-based configuration.
  ...
  return {
    getItem: async (key: string) => {
      const localValue = browserStorage.getItem(key);
      if (localValue) {
        ...
        void request('lovable-preview-auth:get', key).then((res) => {
          ...
          if (res.value === '') browserStorage.removeItem(key);
          else browserStorage.setItem(key, res.value);
        });
        return localValue;
      }
      ...
      if (res && res.ok && typeof res.value === 'string') {
        if (res.value === '') { browserStorage.removeItem(key); return null; }
        return res.value;
      }
